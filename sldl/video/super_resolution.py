...
class VideoSR(nn.Module):
    def __init__(self, model_name='vrt'):
        super(VideoSR, self).__init__()
        self.model_name = model_name
        if model_name == 'vrt':
            self.model = VRT(upscale=4, img_size=[6,64,64], window_size=[6,8,8], depths=[8,8,8,8,8,8,8, 4,4,4,4, 4,4],
            ...
            path = get_checkpoint_path('https://github.com/JingyunLiang/VRT/releases/download/v0.0/001_VRT_videosr_bi_REDS_6frames.pth')
    ...
    def apply_imagesr(self, path):
        frames = get_video_frames(path)
        return [self.model(frame) for frame in tqdm(frames[:480])]
    ...
    def __call__(self, path, dest, ):
        if self.model_name == 'vrt':
            ...
            out_frames = self.apply_imagesr(path)
        fps = get_fps(path)
