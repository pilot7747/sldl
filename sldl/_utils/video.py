...
def get_video_frames(
    path: str, resolution: Optional[Tuple[int, int]] = None
) -> List[Any]:
    ...
    while True:
        ...
        if ret:
            if resolution is None:
                frames.append(
                    Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_RGBA2BGRA)).convert(
                        "RGB"
                    )
                )
            else:
                frames.append(
                    Image.fromarray(cv2.cvtColor(frame, cv2.COLOR_RGBA2BGRA))
                    .convert("RGB")
                    .resize(resolution)
                )
            currentframe += 1
